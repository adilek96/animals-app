        ...
        <div className="xl:flex flex-nowrap  lg:hidden md:hidden sm:hidden">
          <nav className="inline-block">
            <ul className=" box-content text-[16px] font-bold antialiased flex flex-none h-[100px] gap-[3px]   items-center     ">
              ...
              <li>
                <Link href="/posts" onMouseEnter ={showPostsSubMenu} onMouseLeave={hidePostsSubMenu} className={` box-content relative h-[100px]  px-3 flex items-center  hover:text-orange-500 duration-500 ${pathname === '/posts' ? ' text-orange-500 border-t-[5px] border-t-orange-500 border-solid' : ''}`} >Обьявления <FaChevronDown  className="text-2xl ml-1 text-gray-400 pt-2 font-extrabold"/> </Link>
                {postsSubActive ? 
                <ul onMouseEnter={onMouseInPostsSub}  onMouseLeave={onMouseOutPostsSub} className="absolute shadow-xl shadow-orange-900 h-[60px] w-[200px] bg-white  rounded-b-md z-10  ">
                  <li><Link href="/" className={`h-[60px] px-3 flex items-center  hover:text-orange-500 `}>Продавцы</Link></li>
                </ul> : ""}
              </li>
              <li>
                <Link href="/articles" onMouseEnter ={showArticleSubMenu} onMouseLeave={hideArticleSubMenu} className={`box-content relative h-[100px] px-3 flex items-center  hover:text-orange-500 duration-500 ${pathname === '/articles' ? ' text-orange-500 border-t-[5px] border-t-orange-500 border-solid' : ''}`}>Статьи <FaChevronDown  className="text-2xl ml-1 text-gray-400 pt-2 font-extrabold"/></Link>
                {articleSubActive ? 
              ...
              <li><Link href="/clinic" className={`box-content h-[100px] px-3 flex items-center  hover:text-orange-500 duration-300 ${pathname === '/clinic' ? ' text-orange-500 border-t-[5px] border-t-orange-500 border-solid' : ''}`}>Ветклиники</Link></li>
            </ul>
          </nav>
          <button className={`flex h-[100px] px-5  items-center   hover:text-orange-500 duration-300`}><FaSignInAlt className="text-2xl mr-2 text-gray-300  font-extrabold"/>Войти</button>
        </div>
