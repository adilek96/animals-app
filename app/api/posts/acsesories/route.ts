...
//     const offset = 0;
//     const result = await client.query("SELECT ads.*,TO_CHAR(ads.added_date, 'DD/MM/YY HH24:MI') as formatted_added_date, ads_images.image_url FROM ads  LEFT JOIN ads_images ON ads.ad_id = ads_images.ad_id WHERE ads.category = 'acsesories' ORDER BY ads.ad_id DESC LIMIT $1 OFFSET $2", [perPage, offset]);
...
export async function GET(request: Request) {
  ...
  try {
    ...
    let query: string = "SELECT DISTINCT ON (ads.ad_id) ads.*,TO_CHAR(ads.added_date, 'DD.MM.YY HH24:MI') as formatted_added_date, ads_images.image_url FROM ads LEFT JOIN ads_images ON ads.ad_id = ads_images.ad_id WHERE ads.category = 'acsesories' AND " + delivery + " ads.price >= " + sortMinPrice + " AND ads.price <= " + sortMaxPrice + "  ORDER BY ads.ad_id " +  date + "  LIMIT $1 OFFSET $2"
    if(sortPrice === "none"){
      query
    } else if(sortPrice === "down"){
      query = "SELECT DISTINCT ON (ads.price) ads.*,TO_CHAR(ads.added_date, 'DD.MM.YY HH24:MI') as formatted_added_date, ads_images.image_url FROM ads LEFT JOIN ads_images ON ads.ad_id = ads_images.ad_id  WHERE ads.category = 'acsesories' AND " + delivery + " ads.price >= " + sortMinPrice + " AND ads.price <= " + sortMaxPrice + " ORDER BY ads.price DESC  LIMIT $1 OFFSET $2";
    } else if(sortPrice === "up"){
      query ="SELECT DISTINCT ON (ads.price) ads.*,TO_CHAR(ads.added_date, 'DD.MM.YY HH24:MI') as formatted_added_date, ads_images.image_url FROM ads LEFT JOIN ads_images ON ads.ad_id = ads_images.ad_id  WHERE ads.category = 'acsesories' AND " + delivery + " ads.price >= " + sortMinPrice + " AND ads.price <= " + sortMaxPrice + " ORDER BY ads.price ASC  LIMIT $1 OFFSET $2";
    }
