...
export function ThemeSwitch({ settings }: { settings: boolean }) {
  ...
  return (
    <motion.div
      ...
      } duration-300 justify-start items-center ml-3 `}
    >
      <Switch
        checked={enabled}
        onChange={setEnabled}
      ...
      >
        ...
        <span
          ...
          className={`${enabled ? "translate-x-6" : "translate-x-[-2px]"}
            pointer-events-none border-primary-500 border-[2px]  h-[29px] w-[29px] transform rounded-full ${
              enabled ? "bg-white" : "bg-black"
            } shadow-lg ring-0 transition duration-200 ease-in-out flex items-center justify-center`}
        >
          {enabled ? (
            <RxSun className={`text-yellow-400`} />
          ) : (
            <RxMoon className={`text-yellow-400`} />
          )}
        </span>
      </Switch>
    </motion.div>
